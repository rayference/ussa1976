...
def test_create_invalid_variables(
    test_altitudes: npt.NDArray[np.float64],
) -> None:
...
def test_create_below_86_km_layers_boundary_altitudes() -> None:
    ...
    level_pressure: npt.NDArray[np.float64] = np.array(
        [101325.0, 22632.0, 5474.8, 868.01, 110.90, 66.938, 3.9564, 0.37338]
    )
    level_mass_density: npt.NDArray[np.float64] = np.array(
        [
            1.225,
            0.36392,
            0.088035,
...
def test_create_below_86_km_arbitrary_altitudes() -> None:
    ...
    temperatures: npt.NDArray[np.float64] = np.array(
        [
            286.850,
            278.725,
            254.025,
    ...
    pressures: npt.NDArray[np.float64] = np.array(
        [
            98945.0,
            85076.0,
            52239.0,
    ...
    mass_densities: npt.NDArray[np.float64] = np.array(
        [
            1.2017,
            1.0633,
            0.71641,
...
def test_compute_number_density() -> None:
    ...
    values: t.Dict[str, npt.NDArray[np.float64]] = {
        "N2": np.array(
            [
                1.13e20,
                5.547e19,
...
def test_compute_high_altitude_no_mask() -> None:
    """Returns a Dataset."""
    z: npt.NDArray[np.float64] = np.linspace(86e3, 1000e3)
    ds = init_data_set(z=z)
...
def test_compute_low_altitude() -> None:
    """Returns a Dataset."""
    z: npt.NDArray[np.float64] = np.linspace(0.0, 86e3)
    ds = init_data_set(z=z)
